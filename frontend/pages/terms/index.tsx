...
本利用規約（以下「本規約」といいます。）には、株式会社マカセテ（以下「当社」といいます。）の提供するサービス「${
  Const.SERVICE_NAME
}」のご利用にあたり、登録ユーザーの皆様に遵守していただかなければならない事項及び当社と登録ユーザーの皆様との間の権利義務関係が定められております。当該サービスを登録ユーザーとしてご利用になる方は、本規約に同意する前に、必ず全文お読み下さいますようお願い致します。
...
(2)「当社ウェブサイト」とは、そのドメインが「${publicEnv.host.replace(
  'https://',
  ''
)}」である当社が運営するウェブサイト（理由の如何を問わず当社のウェブサイトのドメイン又は内容が変更された場合は、当該変更後のウェブサイトを含みます。）を意味します。
...
(6)「本サービス」とは、当社が提供するという名称「${
  Const.SERVICE_NAME
}」のサービス（理由の如何を問わずサービスの名称又は内容が変更された場合は、当該変更後のサービスを含みます。）を意味します。
...
6.本条に基づき登録ユーザーの登録が取り消された場合、登録ユーザーは、当社の指示に基づき、当社から提供を受けた本サービスに関連するソフトウェア、マニュ${
  Const.SERVICE_NAME
}その他の物につき、返還、廃棄その他の処分を行うものとします。
...
const TermsPage: NextPage = () => {
  return (
    <>
      <NextSeo title={'利用規約'} />
      <div style={{ whiteSpace: 'pre-wrap' }}>{term}</div>
    </>
  )
}
